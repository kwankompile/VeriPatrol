...
class BackendQueueJob:
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, Any], *, config: Config | None = None) -> BackendQueueJob:
        ...
        evidence_mode = _resolve_evidence_mode_from_payload(payload, evidence, config=config)
...
def _has_path_based_evidence(evidence: dict[str, Any]) -> bool:
    """True when evidence holds local file paths (legacy metadata queue schema)."""
    for key in EVIDENCE_IMAGE_TYPES:
...
def _resolve_evidence_mode_from_payload(
    ...
    *,
    ...
    """
    Resolve evidence_mode for a queue job.

    Rules:
    - Explicit stored mode (metadata|upload) is preserved.
    - Legacy jobs with no mode and path-based image metadata → metadata.
    - Legacy jobs with no mode and no paths → current configured mode (default upload).
    """
    raw = payload.get("evidence_mode")
    ...
    if _has_path_based_evidence(evidence):
        return "metadata"

    if config is not None and config.evidence_mode in VALID_EVIDENCE_MODES:
        return config.evidence_mode
    return "upload"


def _normalize_job(job: BackendQueueJob) -> BackendQueueJob:
    job = _normalize_job_log_statuses(_normalize_job_image_statuses(job))
    if not job.evidence_mode or job.evidence_mode not in VALID_EVIDENCE_MODES:
        if _has_path_based_evidence(job.evidence):
            job.evidence_mode = "metadata"
        else:
            job.evidence_mode = "upload"
...
class BackendClient:
    ...
    def _read_token_cache(self) -> dict[str, Any]:
        return self._read_json_file(self.token_cache_path) or {}

    def _camera_identity_from_cache(self) -> tuple[str | None, str | None]:
        cache = self._read_token_cache()
        camera_id = cache.get("camera_id")
        if not camera_id:
            return None, None
        camera_name = cache.get("camera_name")
        return str(camera_id), str(camera_name) if camera_name else None
    ...
    def save_token(
        ...
        *,
        ...
        self._write_json_file(self.token_cache_path, payload)
    ...
    def login(self) -> BackendToken:
        if not self.config.camera_email or not self.config.camera_password:
            raise BackendApiError(
                "Camera email and password are required for login.",
                path="/camera-auth/login",
            )

        body = {
            "email": self.config.camera_email,
        ...
        try:
            ...
            if exc.status_code == 401:
                raise BackendApiError(
                    ...
                    "Verify ANPR_CAMERA_EMAIL and ANPR_CAMERA_PASSWORD.",
                    status_code=401,
                    path="/camera-auth/login",
                ) from exc
            raise
        ...
        if not isinstance(data, dict) or not data.get("access_token"):
            raise BackendApiError(
                "Camera login response did not include access_token.",
                path="/camera-auth/login",
            )
        ...
        if not isinstance(camera, dict) or not camera.get("id"):
            raise BackendApiError(
                "Camera login response did not include camera identity.",
                path="/camera-auth/login",
            )
        ...
        self.save_token(
            ...
            camera_name=str(camera.get("name", "")) or None,
        )
        return token
    ...
    def get_valid_token(self, *, force_refresh: bool = False) -> BackendToken:
        if not force_refresh:
            ...
            if cached is not None and self._token_is_valid(cached):
                camera_id, _ = self._camera_identity_from_cache()
                if camera_id:
                    return cached
        return self.login()
    ...
    def verify_camera_identity(self) -> None:
        """Confirm camera JWT and identity are available (login or valid cache)."""
        if self._camera_verified_for_flush:
            return

        camera_id, _ = self._camera_identity_from_cache()
        cached = self.load_token()
        if camera_id and cached is not None and self._token_is_valid(cached):
        ...
        if not camera_id:
            raise BackendApiError(
                "Camera identity not available after login.",
                path="/camera-auth/login",
            )
        self._camera_verified_for_flush = True

    def _authenticated_camera_id(self) -> str:
        self.get_valid_token()
        camera_id, _ = self._camera_identity_from_cache()
        if not camera_id:
            raise BackendApiError(
                "Camera identity not available.",
                path="/camera-auth/login",
            )
        return camera_id

    def _validate_and_sanitize_job_event(self, job: BackendQueueJob) -> str | None:
        """Return validation error message or None after stripping legacy event fields."""
        authenticated_id = self._authenticated_camera_id()
        queued_id = job.event.get("camera_id")
        if queued_id is not None and str(queued_id) != authenticated_id:
            return "Queued job camera_id does not match authenticated camera."

        sanitized = dict(job.event)
        sanitized.pop("camera_id", None)
        sanitized.pop("vehicle_id", None)
        sanitized.pop("is_flagged", None)
        job.event = sanitized
        return None
    ...
    def read_queue(self) -> tuple[list[BackendQueueJob], int]:
        ...
        for line_number, line in enumerate(
            ...
            if not isinstance(payload, dict):
                ...
                continue
            try:
                jobs.append(BackendQueueJob.from_dict(payload, config=self.config))
    ...
    def _delete_local_evidence_if_configured(self, job: BackendQueueJob) -> None:
        job_evidence_mode = job.evidence_mode
        if job_evidence_mode not in VALID_EVIDENCE_MODES:
            job_evidence_mode = _resolve_evidence_mode_from_payload(
    ...
    def _process_job(
        ...
        try:
            # Prefer the job's stored/inferred mode; never re-read a later .env change
            # for jobs that already carry an explicit evidence_mode.
            job_evidence_mode = job.evidence_mode
            if job_evidence_mode not in VALID_EVIDENCE_MODES:
                job_evidence_mode = _resolve_evidence_mode_from_payload(
                    {"evidence_mode": job.evidence_mode},
                ...
                job.evidence_mode = job_evidence_mode
            if job_evidence_mode not in VALID_EVIDENCE_MODES:
                job.status = "validation_failed"
            ...
            if not backend_event_id:
                validation_error = self._validate_and_sanitize_job_event(job)
                if validation_error:
                    job.status = "validation_failed"
                ...
                if all_jobs is not None and job_index is not None:
                    self._checkpoint_job_if_available(job, all_jobs, job_index)
        ...
        if _has_processable_job(jobs):
            try:
                ...
                if exc.is_validation_failure():
                    ...
                    return FlushQueueResult(
                        success=True,
                        message="Backend camera identity validation failed.",
                        skipped=len(jobs),
                        malformed=malformed,
                        camera_verified=False,
                    )
