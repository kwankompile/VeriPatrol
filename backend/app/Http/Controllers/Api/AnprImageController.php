...
class AnprImageController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $validated = $validator->validated();
            $camera = AnprCameraPrincipal::fromRequest($request);

            if ($camera !== null) {
                $event = AnprEvent::query()->findOrFail($validated['anpr_event_id']);
                $forbidden = AnprCameraPrincipal::forbiddenUnlessOwnsEvent($camera, $event);
            ...
            $anprImage = AnprImage::query()->create($validated);
    ...
    public function uploadForEvent(
        ...
        try {
            ...
            if ($camera !== null) {
                $forbidden = AnprCameraPrincipal::forbiddenUnlessOwnsEvent($camera, $anprEvent);
