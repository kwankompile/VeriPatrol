...
class BlockchainRecordController extends Controller
{
    ...
    public function verifyAll(BlockchainVerificationService $verificationService): JsonResponse
    {
        $validated = request()->validate([
            'status' => ['nullable', Rule::in(['pending', 'queued', 'processing', 'submitted', 'confirmed', 'failed'])],
            'network' => ['nullable', Rule::in(['ganache', 'sepolia'])],
        ...
        $summary = $verificationService->verifyAllEligibleRecords(
            $validated,
            request()->user('api'),
        );
        ...
        if ($summary['total_scanned'] === 0) {
            return response()->json([
                'success' => false,
                'message' => 'No eligible blockchain records matched the selected filters.',
                'data' => $summary,
            ], 422);
        }
