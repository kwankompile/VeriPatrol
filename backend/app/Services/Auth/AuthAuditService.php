...
class AuthAuditService
{
    ...
    public const EVENT_TWO_FACTOR_RESET = 'two_factor_reset';
    ...
    public const EVENT_PROFILE_PICTURE_UPLOADED = 'profile_picture_uploaded';

    public const EVENT_PROFILE_PICTURE_REMOVED = 'profile_picture_removed';

    public const EVENT_EMAIL_CHANGE_STARTED = 'email_change_started';

    public const EVENT_EMAIL_CHANGED = 'email_changed';

    public const EVENT_EMAIL_CHANGE_FAILED = 'email_change_failed';

    public const EVENT_TWO_FACTOR_RECONFIGURE_STARTED = 'two_factor_reconfigure_started';

    public const EVENT_TWO_FACTOR_RECONFIGURED = 'two_factor_reconfigured';

    public const EVENT_TWO_FACTOR_RECONFIGURE_FAILED = 'two_factor_reconfigure_failed';
    ...
    public function record(
        ...
        bool $omitTopLevelEmail = false,
    ): AuthAuditLog {
        return AuthAuditLog::query()->create([
            'user_id' => $user?->getKey(),
            'event_type' => $eventType,
            'status' => $status,
            'email' => $omitTopLevelEmail ? null : ($email ?? $user?->email),
    ...
    private function sanitizeMetadata(?array $metadata): ?array
    {
        ...
        foreach ($metadata as $key => $value) {
            ...
            if (is_string($value) && ! str_ends_with($normalizedKey, '_hash') && $this->containsSensitiveValue($value)) {
                continue;
            }
    ...
    private function containsSensitiveValue(string $value): bool
    {
        ...
        return str_starts_with($normalized, 'bearer ')
            || str_starts_with($normalized, 'otpauth://');
    }
}
