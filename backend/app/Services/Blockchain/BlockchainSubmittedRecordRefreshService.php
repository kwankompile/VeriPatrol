...
class BlockchainSubmittedRecordRefreshService
{
    ...
    public const MSG_TX_NOT_FOUND = 'Submitted transaction was not found by the configured RPC endpoint. '
        .'Possible causes: RPC provider changed or lost mempool history; '
        .'transaction hash may belong to a different network or chain ID; '
        .'transaction may have been dropped, replaced, or never propagated. '
        .'Check network/chain configuration or retry anchoring.';
    ...
    public function scheduleRefresh(BlockchainRecord $record, int $attemptNumber = 1): void
    {
        ...
        $contextTxHash = is_string($record->tx_hash) ? strtolower(trim($record->tx_hash)) : null;
    ...
    {
        $currentTxHash = is_string($record->tx_hash) ? strtolower(trim($record->tx_hash)) : null;

        $query = BlockchainJob::query()
            ->where('blockchain_record_id', $record->id)
            ->where('job_type', 'refresh_confirmation');

        if ($currentTxHash !== null && $currentTxHash !== '') {
            // Check whether any context-aware rows already exist for the current tx_hash.
            // If yes: scope exclusively to those rows — rows for other tx hashes must not
            //   inflate the attempt counter for the current tx cycle.
            // If no:  fall back only to legacy context_tx_hash = null rows (created before
            //   the migration). Rows with a different non-null context_tx_hash belong to a
            //   prior tx cycle and are irrelevant to the current cycle.
            $hasContextAwareRows = BlockchainJob::query()
                ->where('blockchain_record_id', $record->id)
                ->where('job_type', 'refresh_confirmation')
                ->where('context_tx_hash', $currentTxHash)
                ->exists();

            if ($hasContextAwareRows) {
                $query->where('context_tx_hash', $currentTxHash);
            } else {
                // New tx cycle: no context-aware rows exist for this tx_hash yet.
                // Start at attempt 1 — do not inherit legacy null rows from prior tx cycles.
                return 1;
            }
        }

        $latestAttempt = $query->max('attempts');

        $next = max(1, (int) $latestAttempt + 1);
        $maxAttempts = $this->retryService->maxAttempts();

        return min($next, $maxAttempts);
    }
