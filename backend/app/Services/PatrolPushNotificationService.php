...
class PatrolPushNotificationService
{
    ...
    public function checkpointStatusAlert(CheckpointEvent $checkpointEvent, ?string $previousStatus = null): void
    {
        try {
            ...
            if (! in_array($status, ['suspicious', 'needs_review', 'uncertain'], true)) {
                return;
            }
            ...
            $title = $status === 'suspicious' ? 'Suspicious checkpoint' : 'Checkpoint needs review';
    ...
    public function validationCompleted(PatrolSession $session, array $validationResult): void
    {
        try {
            ...
            $partial = $results->where('status', 'partial')->count();
            $needsReview = $results->whereIn('status', ['needs_review', 'uncertain'])->count();
            $suspicious = $results->where('status', 'suspicious')->count();
            $missed = $results->whereIn('status', ['missed', 'rejected'])->count();

            $summary = "Verified {$verified}, partial {$partial}, needs review {$needsReview}, suspicious {$suspicious}, missed {$missed}.";
            ...
            if ($missed > 0 && $session->user_id) {
                $user = User::query()->find($session->user_id);
                if ($user) {
                    $this->webPush->sendToUser($user, $this->payload(
                        'Checkpoint(s) missed',
                        "{$missed} checkpoint(s) had insufficient evidence after validation. Review your patrol summary.",
                        '/patrol',
                        'patrol-missed-'.$sessionId,
                        ['patrol_session_id' => $sessionId]
                    ));
                }
            }
