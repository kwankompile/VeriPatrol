...
class AuthTwoFactorTest extends TestCase
{
    ...
    public function test_two_factor_secret_column_is_wide_enough_for_encrypted_payload(): void
    {
        $encryptedSecret = encrypt(str_repeat('A', 32));

        $this->assertGreaterThan(255, strlen($encryptedSecret));

        $columnType = Schema::getColumnType('users', 'two_factor_secret');

        $this->assertContains($columnType, ['text', 'mediumtext', 'longtext']);
    }
