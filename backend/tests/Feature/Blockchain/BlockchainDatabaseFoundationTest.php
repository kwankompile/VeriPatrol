...
class BlockchainDatabaseFoundationTest extends TestCase
{
    ...
    public function test_unique_proof_rule_prevents_duplicate_records_with_same_hash(): void
    {
        $recordHash = hash('sha256', 'duplicate-proof-demo');
        $attributes = [
    ...
    public function test_unique_proof_rule_allows_repeated_entity_proofs_with_different_hashes(): void
    {
        $shared = [
            'entity_type' => 'user_profile',
            'entity_id' => '01940000-0000-7000-8000-000000000199',
            'proof_type' => 'profile_password_changed',
            'canonical_version' => 'v1',
            'environment' => 'local',
        ];

        BlockchainRecord::factory()->create(array_merge($shared, [
            'record_hash' => hash('sha256', 'profile-proof-1'),
        ]));
        BlockchainRecord::factory()->create(array_merge($shared, [
            'record_hash' => hash('sha256', 'profile-proof-2'),
        ]));
    ...
    public function test_security_operator_cannot_access_blockchain_record_endpoints(): void
    {
