...
class BlockchainVerificationTest extends TestCase
{
    ...
    public function test_security_operator_cannot_manually_verify_a_record(): void
    {
        $this->fakeVerifyRpc(found: true);
    ...
    public function test_admin_can_verify_all_confirmed_records(): void
    {
        $this->fakeVerifyRpc(found: true);
        ...
        $this->actingAs($admin, 'api')
            ->postJson('/api/blockchain-records/verify-all')
            ->assertOk()
            ->assertJsonPath('data.total_scanned', 1)
            ->assertJsonPath('data.valid', 1)
            ->assertJsonPath('data.tampered_records', []);
    }

    public function test_verify_all_returns_tampered_record_details(): void
    {
        $admin = $this->adminUser();
        $record = $this->createConfirmedAnprRecord();
        ...
        $response = $this->actingAs($admin, 'api')
            ...
            ->assertOk()
            ->assertJsonPath('data.total_scanned', 1)
            ->assertJsonPath('data.tampered', 1)
            ->assertJsonPath('data.valid', 0);

        $this->assertCount(1, $response->json('data.tampered_records'));
        $this->assertSame($record->id, $response->json('data.tampered_records.0.record_id'));
        $this->assertSame('tampered', $response->json('data.tampered_records.0.result'));
        $this->assertNotEmpty($response->json('data.tampered_records.0.stored_hash'));
        $this->assertNotEmpty($response->json('data.tampered_records.0.recomputed_hash'));
    }

    public function test_verify_all_returns_422_when_no_eligible_records(): void
    {
        $admin = $this->adminUser();
        ...
        $this->actingAs($admin, 'api')
            ->postJson('/api/blockchain-records/verify-all')
            ->assertStatus(422)
            ->assertJsonPath('data.total_scanned', 0);
    }

    public function test_security_operator_cannot_verify_all(): void
    {
        $operator = $this->securityOperatorUser();
