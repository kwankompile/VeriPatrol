...
class BlockchainRetryServiceTest extends TestCase
{
    ...
    public function test_stale_retry_reason_allows_manual_admin_retry_despite_attempt_mismatch(): void
    {
        $record = BlockchainRecord::factory()->failed()->create([
            'retry_count' => 10,
        ]);

        $manualJob = BlockchainJob::factory()->for($record)->create([
            'job_type' => 'retry_anchor',
            'status' => 'queued',
            'attempts' => 1,
        ]);

        $this->assertNull($this->service->staleRetryReason($record, $manualJob, isManualAdminRetry: true));
        $this->assertSame(
            BlockchainRetryService::STALE_RETRY_REASON,
            $this->service->staleRetryReason($record, $manualJob, isManualAdminRetry: false),
        );
    }
