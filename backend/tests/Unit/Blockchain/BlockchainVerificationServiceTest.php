...
class BlockchainVerificationServiceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->service = new BlockchainVerificationService(
            ...
            app(PatrolValidationService::class),
        );
    }
    ...
    public function test_confirmed_user_profile_record_with_matching_payload_summary_returns_valid(): void
    {
        $this->fakeVerifyRpc(found: true);
    ...
    public function test_tampered_user_profile_payload_summary_returns_tampered(): void
    {
        Http::fake();

        $payload = [
            'entity_type' => 'user_profile',
            'entity_id' => '01940000-0000-7000-8000-000000000404',
            'proof_type' => 'profile_email_changed',
            'profile_version' => 5,
            'changed_at' => '2026-06-29T10:00:00Z',
            'actor_user_id' => '01940000-0000-7000-8000-000000000404',
            'revoked_count' => 1,
            'source' => 'self_profile',
    ...
    public function test_user_profile_record_with_invalid_payload_summary_returns_failed(): void
    {
        Http::fake();

        $record = BlockchainRecord::factory()->confirmed()->create([
            'entity_type' => 'user_profile',
            'entity_id' => '01940000-0000-7000-8000-000000000505',
            'proof_type' => 'profile_password_changed',
            'record_hash' => hash('sha256', 'invalid-summary'),
            'payload_summary' => ['module' => 'profile'],
            'contract_address' => '0x'.str_repeat('a', 40),
        ]);
        ...
        $this->assertSame('failed', $verification->result);
        $this->assertStringContainsString('payload summary', strtolower((string) $verification->error_message));
        Http::assertNothingSent();
    }

    public function test_confirmed_patrol_session_record_with_matching_validation_data_returns_valid(): void
    {
        $this->fakeVerifyRpc(found: true);

        $record = $this->createConfirmedPatrolSessionRecord();
    ...
    public function test_tampered_patrol_checkpoint_event_returns_tampered(): void
    {
        Http::fake();

        $record = $this->createConfirmedPatrolSessionRecord();
        $event = \App\Models\CheckpointEvent::query()
            ->where('patrol_session_id', $record->entity_id)
            ->firstOrFail();
        $event->update(['status' => 'suspicious']);
    ...
    public function test_missing_patrol_session_returns_failed(): void
    {
        Http::fake();

        $record = BlockchainRecord::factory()->confirmed()->create([
            'entity_type' => 'patrol_session',
            'entity_id' => '01940000-0000-7000-8000-000000009999',
            'proof_type' => 'validation_result',
            'record_hash' => hash('sha256', 'missing-patrol'),
            'contract_address' => '0x'.str_repeat('a', 40),
        ]);
        ...
        $this->assertSame('failed', $verification->result);
        $this->assertStringContainsString('patrol session', strtolower((string) $verification->error_message));
        Http::assertNothingSent();
    }
    ...
    private function createConfirmedPatrolSessionRecord(): BlockchainRecord
    {
        $user = User::factory()->create();
        ...
        $movementSummary = [
            'total_location_logs' => (int) $validationResult['total_location_logs'],
            'total_gaps' => (int) $validationResult['total_gaps'],
        ...
        return BlockchainRecord::factory()->confirmed()->create([
            ...
            'hash_algorithm' => 'sha256',
            'record_hash' => $hash['record_hash'],
            'payload_summary' => app(BlockchainHashService::class)->buildPatrolSessionValidationPayloadSummary($payload),
            'contract_address' => '0x'.str_repeat('a', 40),
        ]);
    }
