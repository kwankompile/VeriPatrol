...
**Status:** Implemented (historical M1 scope).
...
| `ANPR_CAMERA_EMAIL` | str | Camera login email (when backend enabled) |
| `ANPR_CAMERA_PASSWORD` | str | Camera login password (when backend enabled) |
| `ANPR_RTSP_URL` | str | RTSP URL; required when backend enabled (sent at camera login) |

**Deprecated (ignored):** `ANPR_BACKEND_EMAIL`, `ANPR_BACKEND_PASSWORD`, `ANPR_BACKEND_CAMERA_ID` — replaced by camera-authenticated login (M3).
...
- When `ANPR_BACKEND_ENABLED=true`, require:
  ...
  - `ANPR_RTSP_URL` (plausible RTSP URL — reported during camera login)
  - `ANPR_BACKEND_TOKEN_CACHE`
  - `ANPR_BACKEND_QUEUE_FILE`
- `.cache/` must exist or be creatable
- Deprecated env vars (`ANPR_BACKEND_EMAIL`, `ANPR_BACKEND_PASSWORD`, `ANPR_BACKEND_CAMERA_ID`) emit warnings if set

**Current implementation:** full camera login, token cache, and queue flush are implemented in `backend.py` (M7+).
