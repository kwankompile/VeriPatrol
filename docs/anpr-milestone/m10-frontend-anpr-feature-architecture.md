...
The feature follows the existing frontend Clean Architecture pattern under `frontend/src/feature/anpr-monitoring/`.
...
- Minimal README milestone update in `anpr`
...
`anpr/README.md` milestone line updated to M10 with a short pointer to this document. Essential setup and CLI content preserved.
...
13. Run `yarn lint` and `yarn build` in `frontend`
14. Run `php artisan test` in `backend`
