...
`backend.py` continues posting (camera identity from camera JWT — **no `camera_id` in payload**):

```text
plate_number, confidence, detection_time, is_valid, latitude, longitude
```
...
| Repo | Key files |
| ---- | --------- |
| `backend` | `AnprVehicleLinker.php`, `AnprEventController.php`, `VehicleController.php`, `routes/api.php`, `AnprVehicleLinkingTest.php` |
| `frontend` | `feature/management-vehicle/*`, `AnprEventSummaryCards.jsx`, routes, menu |
| `anpr` | `README.md`, `docs/m13-linked-vehicle-record-architecture.md`, `ai-anpr-modules.md` |
