...
| Layer | Coverage |
| ----- | -------- |
| **AI runtime (`anpr`)** | Config validation, source resolution, plate normalization, tracking/voting, finalization, duplicate cooldown, backend queue/token behavior, RTSP reconnect helpers, CLI integration with mocked processor |
...
anpr/
  ...
  tests/
    ...
    test_integration.py
    test_m15_performance_tuning.py
```
...
| Backend | `_is_retryable_job`, `BackendToken` cache validity, camera login payload, queue read/write, flush success/failure/exhausted/skipped, AI payload excludes `camera_id`/`vehicle_id` |
...
| 4 | AI RTSP live backend-enabled run | Backend running; `ANPR_BACKEND_ENABLED=true`; camera credentials in `.env` (`ANPR_CAMERA_EMAIL`, `ANPR_CAMERA_PASSWORD`); successful `POST /api/camera-auth/login`; upload or metadata mode configured | `python main.py run --source rtsp --max-seconds 60 --strict` (no `--dry-run`) | Laravel receives ANPR event; evidence registered per mode | Backend `anpr_events` row; `anpr_images` rows or upload paths | | |
...
| Component | Command |
| --------- | ------- |
| AI unit + integration | `cd anpr && python -m pytest` |
| AI config check | `cd anpr && python main.py check-config` |
| Backend ANPR only | `cd backend && php artisan test --filter=Anpr` |
| Backend full suite | `cd backend && php artisan test` |
