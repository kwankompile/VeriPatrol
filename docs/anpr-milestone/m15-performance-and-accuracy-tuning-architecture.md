...
| Layer | M15 work |
| ----- | -------- |
| **AI (`anpr`)** | OCR throttle, runtime metrics in `worker_summary.json`, tuning profile, performance target results, plate normalization hardening |
| **Backend (`backend`)** | Lighter ANPR list queries (`withCount('images')`), detail endpoint unchanged |
| **Frontend (`frontend`)** | Polling backoff on repeated failures, `images_count` normalization for list evidence badges |
