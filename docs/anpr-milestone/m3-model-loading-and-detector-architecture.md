...
| `detect_plates()` | Padded vehicle-crop detection with full-frame fallback and coordinate translation |
...
- Confidence: `ANPR_PLATE_CONF` (default `0.25`; use `0.20`–`0.25` for motorcycle scenes)
- With `vehicle_detection`: crops a **padded** vehicle region (configurable padding; motorcycles use a larger default), detects plates, translates boxes to full-frame coordinates
- **Per-track full-frame fallback**: when any eligible track has zero crop plate detections, runs one full-frame plate pass and associates results only to missed tracks (with duplicate filtering against crop detections)
- Records crop/fallback call counts, detections by source, and elapsed milliseconds
- Set `ANPR_DEBUG_DETECTIONS=true` to save annotated debug frames under `runs/<run_id>/debug/`
