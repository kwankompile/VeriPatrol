...
Multiple OCR fragments are assembled top-to-bottom, then left-to-right (not by confidence). Two-line motorcycle plates such as `JWG` + `2405` assemble to `JWG2405`. Position-aware OCR confusion correction (`correct_plate_ocr_confusions`) runs before validation.
