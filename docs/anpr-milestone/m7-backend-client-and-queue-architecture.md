...
- `POST /api/camera-auth/login` with camera JWT reuse and 401 refresh (M3)
...
### Out of Scope (at M7 delivery)

- ~~Binary image upload~~ — **implemented in M9** (`ANPR_EVIDENCE_MODE=upload`)
...
**Current:** both `metadata` and `upload` evidence modes are valid when backend is enabled.
...
| `ANPR_CAMERA_EMAIL` / `ANPR_CAMERA_PASSWORD` | Camera login credentials (M3) |
| `ANPR_RTSP_URL` | RTSP source; sent during camera login when backend enabled |
...
| `ANPR_EVIDENCE_MODE` | `metadata` or `upload` (upload recommended for cloud deployment) |

`flush-backend-queue` validates only backend settings via `validate_backend_config()` — not YOLO models, OCR, or source media.

## Evidence Mode Notes

When `ANPR_EVIDENCE_MODE=upload` and backend is enabled, evidence files are uploaded via `POST /api/anpr-events/{id}/images/upload`.

When `ANPR_EVIDENCE_MODE=metadata`, the runtime posts image metadata paths via `POST /api/anpr-images`; Laravel must resolve paths through `ANPR_IMAGE_ROOTS`.
...
- Reused when `expires_at` is more than 60 seconds in the future **and** `camera_id` is present
- Legacy cache files without `camera_id` force a fresh camera login before posting
...
- Metadata mode stores paths only; backend must resolve files locally via `ANPR_IMAGE_ROOTS`
...
- Camera identity requires valid `ANPR_CAMERA_EMAIL` / `ANPR_CAMERA_PASSWORD` and successful `POST /api/camera-auth/login`
- Legacy queue jobs with a mismatched `event.camera_id` are marked `validation_failed` (M3)

## Next Milestone Handoff Notes for M8/M9

**M8** — Backend data alignment: camera JWT identity, field mapping, event logs, and dashboard display against posted records.

**M9** — Evidence delivery: upload mode, retention policy, `ANPR_DELETE_LOCAL_AFTER_UPLOAD`, and local evidence cleanup after upload (**complete**).
